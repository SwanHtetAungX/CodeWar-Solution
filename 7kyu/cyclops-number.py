...
#MySolution
def cyclops(n):
    binary = bin(n)[2:]  # Convert n to binary and remove the leading "0b"
    if len(binary) % 2 == 0 or binary.count("0") != 1:
        return False  # Check that the binary representation has an odd length and exactly one "0"
    middle_index = len(binary) // 2
    return binary[middle_index] == "0"  # Check that the middle digit is "0"
